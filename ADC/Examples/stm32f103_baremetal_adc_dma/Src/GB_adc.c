...
{
//	{
//		if((DMA1->ISR)&(1<<11)) //DMA receive complete
//			{
...
void dma_init()
{
    ...
    DMA1_Channel1->CCR |= (DMA_CCR_MSIZE_1); // memory size transfer set as 16 bits

    DMA1_Channel1->CCR |= (DMA_CCR_PSIZE_1); // peripheral size transfer set as 16 bits
...
void dma_adc_convert(uint32_t srcadd, uint32_t destadd, uint16_t datasize)
{
	...
	//dma_init(); //initliaze the dma clock
